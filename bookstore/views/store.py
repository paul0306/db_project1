...
def bookstore():
    ...
    if 'keyword' in request.args and 'page' in request.args:
    ...
    elif 'dname' in request.args:
        dname = request.args['dname']
        rname = request.args['branch_select']
        ...
        if(len(dish_data) < end):
            end = len(dish_data)
            flag = 1
            
        for j in range(start, end):
            final_data.append(dish_data[j])
        
        return render_template('bookstore.html', selected_branch=selected_branch, final_data=dish_data, 
                               user=current_user.name, page=page, flag=flag, count=count)    
        ...
        selected_branch = request.form.get('branch_select', '中山店')
        branch_data = branch()
        dish_data = dish(selected_branch)
        # dish_data = []
        # temp = 0
        ...
        return render_template('bookstore.html', selected_branch=selected_branch, branch_data=branch_data, 
                               dish_data=dish_data, user=current_user.name, page=1, flag=flag, count=count)
...
@store.route('/reservation', methods=['GET', 'POST'])
def reservation():
    if request.method == 'POST':
        Reservation.add_reservation(
            {
                'rsname': request.values.get('rsname'),
                'rname': request.values.get('rname'),
                'rsdate': request.values.get('rsdate'),
                'rsphone': request.values.get('rsphone'),
                'rsamount': request.values.get('rsamount'),
            }
        )
